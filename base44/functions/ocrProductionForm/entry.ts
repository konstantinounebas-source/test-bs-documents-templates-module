...
Deno.serve(async (req) => {
  ...
  // ── Single-pass OCR + validation ────────────────
   const result = await base44.asServiceRole.integrations.Core.InvokeLLM({
     model: model,
     prompt: `Εξάγαγε δεδομένα ΜΟΝΟ από τη φόρμα "ΗΜΕΡΗΣΙΑ ΠΑΡΑΓΩΓΗ" ή "Διεργασία1: Προετοιμασία Βαφής".${pageInstruction}

  ⚠️ ΚΡΙΣΙΜΟ - ΑΠΟΦΥΓΗ ΛΑΘΟΥΣ:
  - ΜΗΝ εξάγεις δεδομένα από τη φόρμα "PRODUCTION TEAMS TIME FORM V.4" ή κάποια φόρμα με πίνακα ωρών εργασίας.
  - Η σωστή φόρμα έχει πίνακα με ΚΩΔΙΚΟΥΣ ΚΟΜΜΑΤΙΩΝ (item codes) και στοιχεία παραγωγής (QC stock, treatments, hanging, etc.).
  - Εάν υπάρχουν πολλές σελίδες και ο τίτλος μιας σελίδας δεν είναι "ΗΜΕΡΗΣΙΑ ΠΑΡΑΓΩΓΗ" ή "Διεργασία1", ΑΓΝΟΗΣΕ εκείνη τη σελίδα.
