...
export default function DailySupervisorCostsSection({ entries, selectedDate, unitCost, formatCurrency, onAdd, onRemove, onUpdate }) {
    const visibleWithIdx = selectedDate
        ? entries.map((r, i) => ({ r, i })).filter(({ r }) => r.date === selectedDate && r.cost_type === 'supervisor')
        : entries.map((r, i) => ({ r, i })).filter(({ r }) => r.cost_type === 'supervisor');
    ...
    const handleUpdate = (realIdx, field, value) => {
        ...
        updated[realIdx] = row;
        console.log('✅ Updating supervisor cost row:', updated[realIdx]);
        onUpdate(updated);
    };
    ...
    const handleAdd = () => {
        ...
        console.log('✅ Adding supervisor cost row:', newRow);
        onAdd(newRow);
    };
        ...
        <Card>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <CardTitle className="text-base font-semibold text-slate-800">
                            Ε. Κόστη Επιστάρχη (Ημερήσια)
                        </CardTitle>
                    </div>
                    <div className="flex items-center gap-3">
                        <span className="text-sm text-slate-500">
                            Ημερήσιο Κόστος: <strong className="text-purple-700">{formatVal(unitCost)}</strong>
                        </span>
                        <span className="text-sm text-slate-500">
            ...
            <CardContent>
                {visibleWithIdx.length === 0 ? (
                    <p className="text-sm text-slate-400 text-center py-6">Δεν υπάρχουν εγγραφές κοστών επιστάρχη για {selectedDate || 'αυτή την ημέρα'}. Κάντε κλικ στο «Προσθήκη».</p>
                ) : (
