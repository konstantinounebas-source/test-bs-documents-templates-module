...
export default function ImportReceiptFromFileDialog({ isOpen, onClose, onItemsImported, stops, stickerItems, stickerTemplates }) {
  ...
  const validateAllItems = async () => {
    ...
    const results = importedData.map((item, idx) => {
      ...
      // For receipts, we need items that are "Ordered" status
      const matchingStickerItems = (stickersByStopId.get(stop.id) || []).filter(si => si.status === "Ordered");

      if (matchingStickerItems.length === 0) {
        return {
          isValid: false,
          error: `No ordered stickers for Stop ID "${item.stop_id}"`
        };
      }
    ...
    <>
      ...
      <Dialog open={step !== "idle"} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Import Receipt Data</DialogTitle>
          </DialogHeader>

          {step === "idle" && (
            <div className="space-y-4 py-4">
              <p className="text-gray-600">Load the receipt template file to import received sticker quantities.</p>
