...
// Note: Info is imported from lucide-react above
...
export function AddDialog({ open, onOpenChange, formData, setFormData, profiles, qcTypes, qcLevels, itemCodes, onAdd, isPending }) {
  return (
...
export function EditDialog({ open, onOpenChange, editingRecord, setEditingRecord, onSave }) {
...
export function DetailsDialog({ open, onOpenChange, selectedRecord, details }) {
      ...
      <DialogContent className="max-w-3xl">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            {selectedRecord.notes && <div className="col-span-2"><Label className="text-slate-600">Notes</Label><p className="font-medium">{selectedRecord.notes}</p></div>}
          </div>
          <Card>
            <CardHeader><CardTitle className="text-base">Operations Breakdown</CardTitle></CardHeader>
            <CardContent>
              <Table>
                <TableHeader><TableRow><TableHead>Operation</TableHead><TableHead className="text-right">Minutes per Piece</TableHead></TableRow></TableHeader>
                <TableBody>
          ...
          {details.qcBreakdown.length > 0 && (
            <Card>
              ...
              <CardContent>
                <Table>
                  <TableHeader><TableRow><TableHead>Operation</TableHead><TableHead className="text-right">Extra Minutes per Piece</TableHead></TableRow></TableHeader>
                  <TableBody>
            ...
            <CardContent className="pt-4">
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  ...
                  <p className="text-xs text-slate-500">({(selectedRecord.ops_per_piece_min||0).toFixed(2)} × {selectedRecord.ops_qty})</p>
                </div>
                <div>
                  <p className="text-sm text-slate-600">QC Total</p>
                  <p className="text-lg font-bold">{(selectedRecord.qc_total_min || 0).toFixed(2)} min</p>
                  <p className="text-xs text-slate-500">({(selectedRecord.qc_per_piece_min||0).toFixed(2)} × {selectedRecord.qc_qty})</p>
                </div>
                <div>
...
export function SaveTemplateDialog({ open, onOpenChange, templateName, setTemplateName, selectedDate, onSave }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
...
function Info({ className }) { return <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>; }

export function LoadTemplateDialog({ open, onOpenChange, loadToDate, setLoadToDate, templates, onLoad }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
              ...
              {templates.length === 0 ? (
                ...
                templates.map(template => (
                  <div key={template.id} className="p-4 hover:bg-slate-50 flex justify-between items-center">
                    ...
                    <Button onClick={() => onLoad(template)} disabled={!loadToDate} size="sm">Load</Button>
                  </div>
                ))
              )}
            </div>
...
export function AssignPersonsDialog({ open, onOpenChange, selectedDate, dayPersons, setDayPersons, dayNotes, setDayNotes, newPersonName, setNewPersonName, newPersonMinutes, setNewPersonMinutes, persons, onSave, currentDayAssignment }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-auto">
        <DialogHeader><DialogTitle>Assigned Persons - {selectedDate}</DialogTitle></DialogHeader>
        <div className="space-y-4 py-4">
          <div className="flex gap-2 items-end">
            ...
            <Button onClick={() => {
              ...
              if (dayPersons.find(p => p.name === newPersonName.trim())) return;
              setDayPersons([...dayPersons, { name: newPersonName.trim(), available_minutes: newPersonMinutes }]);
              setNewPersonName(''); setNewPersonMinutes(465);
            }}>
            ...
            {dayPersons.length === 0 ? (
              ...
              <div key={index} className="p-3 flex justify-between items-center hover:bg-slate-50">
                ...
                <div className="flex gap-2">
                  <Input type="number" value={person.available_minutes} onChange={(e) => { const u=[...dayPersons]; u[index].available_minutes=parseFloat(e.target.value)||0; setDayPersons(u); }} className="w-24" />
                  <Button onClick={() => setDayPersons(dayPersons.filter((_, i) => i !== index))} variant="ghost" size="icon"><Trash2 className="w-4 h-4 text-red-500" /></Button>
                </div>
              </div>
            ))}
          </div>
          {dayPersons.length > 0 && (
            <div className="bg-slate-100 p-3 rounded-lg">
              <p className="text-sm font-semibold">
                Total Available: {dayPersons.reduce((s, p) => s + p.available_minutes, 0)} min
                ({Math.floor(dayPersons.reduce((s,p)=>s+p.available_minutes,0)/60)}h {dayPersons.reduce((s,p)=>s+p.available_minutes,0)%60}m)
              </p>
            </div>
          )}
          <div>
