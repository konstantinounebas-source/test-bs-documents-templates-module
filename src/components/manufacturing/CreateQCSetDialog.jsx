...
export default function CreateQCSetDialog({ open, onOpenChange }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await base44.entities.QC_Set.create(formData);
      queryClient.invalidateQueries(['QC_Set']);
      onOpenChange(false);
      setFormData({ version_no: "", status: "DRAFT", notes: "" });
    } catch (error) {
      console.error("Failed to create QC set:", error);
      alert("Failed to create QC set");
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Create New QC Set</DialogTitle>
        </DialogHeader>
