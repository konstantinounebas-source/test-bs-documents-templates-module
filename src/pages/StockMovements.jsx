...
export default function StockMovementsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [
        movementsData, allMovementsData, productsData, locationsData,
        systemUsers, appUsers, vendorsData, pvData,
        categoriesData, companiesData, purchaseOrdersData
      ] = await Promise.all([
  ...
  const handleImportComplete = () => { setShowImportDialog(false); loadData(); };
  const handleView = (movement) => { setSelectedMovement(movement); setShowViewDialog(true); };
  const handleEdit = (movement) => { setSelectedMovement(movement); setShowEditDialog(true); };
  const handleViewProductMovements = (product) => { setSelectedProductForDrawer(product); setShowProductDrawer(true); };
  ...
  const handleFixOutMovementCosts = async () => {
    if (!window.confirm('Θα ενημερωθούν όλα τα OUT movements που δεν έχουν unit_cost με το τρέχον κόστος μονάδας του προϊόντος. Συνέχεια;')) return;
    setIsFixingCosts(true);
    try {
      const outMovements = movements.filter(m => m.movement_type === 'OUT' && (!m.unit_cost || parseFloat(m.unit_cost) === 0));
      let updated = 0;
      for (const movement of outMovements) {
        const product = products.find(p => p.id === movement.product_id);
        if (product?.unit_cost > 0) {
          await base44.entities.StockMovement.update(movement.id, { unit_cost: product.unit_cost });
          updated++;
        }
  ...
  const calculateDisplayQuantity = (movement) => {
    if (movement.base_quantity && movement.base_quantity > 0) return movement.base_quantity;
  ...
  const exportToCSV = () => {
    const headers = ["Date/Time","Type","Product","SKU","Quantity","From Location","To Location","Charged To","Waybill","Performed By","Reference Type","Reference ID","Notes"];
    const rows = filteredMovements.map(movement => {
      ...
      return [
        ...
        movement.movement_type, product?.name || 'Unknown', product?.sku || 'N/A',
        ...
        movement.reference_type || '-', movement.reference_id || '-', movement.notes || '-'
      ];
    });
  ...
  const getTimeFilterDate = () => {
    ...
    if (timeFilter === 'week') { const d = new Date(now); d.setDate(d.getDate() - 7); return d; }
    if (timeFilter === 'month') { const d = new Date(now); d.setMonth(d.getMonth() - 1); return d; }
    return null;
  };
  ...
  const filteredMovements = movements.filter(movement => {
    ...
    const matchesLocation = locationFilter === "all" || movement.from_location === locationFilter || movement.to_location === locationFilter;
    const matchesProduct = productFilter === "all" || movement.product_id === productFilter;
    const matchesTime = timeFilter === "all" || (getTimeFilterDate() && new Date(movement.created_date) >= getTimeFilterDate());
  ...
  const paginatedMovements = (hasFilters || showAll)
    ? (showAll ? filteredMovements : filteredMovements.slice((currentPage - 1) * parseInt(itemsPerPage), currentPage * parseInt(itemsPerPage)))
    : filteredMovements;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={handleFixOutMovementCosts} disabled={isFixingCosts}>
              {isFixingCosts ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Ενημέρωση...</> : 'Διόρθωση Κόστους OUT'}
            </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            { label: 'Total Movements', value: stats.total, color: 'text-slate-900', icon: 'text-blue-600' },
            { label: 'Stock In', value: stats.in, color: 'text-green-600', icon: 'text-green-600' },
            { label: 'Stock Out', value: stats.out, color: 'text-red-600', icon: 'text-red-600' },
            { label: 'Adjustments', value: stats.adjustments, color: 'text-orange-600', icon: 'text-orange-600' },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <Select value={locationFilter} onValueChange={setLocationFilter}>
                ...
                <SelectContent>
                  ...
                  {locations.filter(l => l.name).map(l => <SelectItem key={l.id} value={l.name}>{l.name}</SelectItem>)}
                </SelectContent>
              </Select>
              <Select value={productFilter} onValueChange={setProductFilter}>
                ...
                <SelectContent>
                  ...
                  {products.filter(p => p.name).map(p => <SelectItem key={p.id} value={p.id}>{p.name} ({p.sku})</SelectItem>)}
                </SelectContent>
              </Select>
      ...
      <ImportStockMovementsDialog open={showImportDialog} onClose={() => setShowImportDialog(false)} onImportComplete={handleImportComplete} />

      <ViewMovementDialog
        open={showViewDialog}
        onClose={() => { setShowViewDialog(false); setSelectedMovement(null); }}
      ...
      <EditMovementDialog
        open={showEditDialog}
        onClose={() => { setShowEditDialog(false); setSelectedMovement(null); }}
        movement={selectedMovement}
        products={products}
        vendors={vendors}
        productVendors={productVendors}
