...
import { recalculateStockForProduct } from "@/utils/recalculateStock";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { ScanBarcode, Package, CheckCircle, AlertTriangle, ArrowRight, ArrowLeft, Camera, X, Plus, Zap, Upload, ShoppingCart } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Separator } from "@/components/ui/separator";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import POReceiveDialog from "../components/warehouse/POReceiveDialog";
...
export default function BarcodeScannerPage() {
  ...
  const handleSubmitBulkInvoice = async () => {
    ...
    try {
      for (const item of bulkInvoiceItems) {
        ...
        // Recalculate stock from movements (single source of truth, allows negatives)
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-6">
      ...
      {/* PO Selection Dialog */}
      <POReceiveDialog
        open={showPOSelectionDialog}
        onOpenChange={setShowPOSelectionDialog}
        selectedPOForBulkReceive={selectedPOForBulkReceive}
        poItemsToReceive={poItemsToReceive}
        setPOItemsToReceive={setPOItemsToReceive}
        toLocation={toLocation}
        setToLocation={setToLocation}
        showPOSummary={showPOSummary}
        setShowPOSummary={setShowPOSummary}
        ...
        onProceedToSummary={handleProceedToPOSummary}
        onConfirm={handleBulkReceiveFromPO}
        onCancel={() => {
          setShowPOSelectionDialog(false);
          setSelectedPOForBulkReceive(null);
          setPOItemsToReceive([]);
